...
def gather_info(task, timestamp):
    try:
        ...
        for c in cmds:
            ...
            logging.info(
                f"Collected structured output for command '{c}'"
                f"from {task.host.name} ({task.host.hostname})")

        filename = f"outputs/{task.host.name}_structured_{timestamp}.yaml"
        with open(filename, "w") as f:
            yaml.safe_dump(outputs, f, default_flow_style = False)
        os.chmod(filename, 0o444)
        logging.info(f"Structured output saved for {task.host.name} ({task.host.hostname}): {filename}")

        return Result(
            host = task.host,
            result = f"Structured data collected from {len(cmds)} commands from {task.host.name} ({task.host.hostname})" 
        )
